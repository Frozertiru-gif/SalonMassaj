...
export function HomeBookingForm({ services }: HomeBookingFormProps) {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      ...
      setSlotsLoaded(false);
      try {
        const response = await fetch(`/api/public/bookings/slots?service_id=${selectedServiceId}&date=${selectedDate}`);
        if (!response.ok) {
          setSlots([]);
          setSlotsLoaded(true);
          return;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        {selectedServiceId && selectedDate && slotsLoaded && slots.length === 0 ? (
          <p className="mt-2 text-xs text-rose-600">Нет доступного времени на выбранную дату. Выберите другую дату.</p>
        ) : null}
      </div>
      <div>
      ...
      {formSent ? (
        <div className="rounded-2xl bg-blush-50 px-4 py-3 text-center text-xs text-blush-700">Запись принята, мы свяжемся с вами</div>
      ) : null}
    </form>
  );
}
